...
instance subtype.comm_ring [S : subring cR] : comm_ring (subtype S) := S.subset_comm_ring cR
...
def subring.inter [S₁ : subring R] [S₂ : subring R] (S₁ ∩ S₂) :
  subring R := 
{ carrier := (S₁ ∩ S₂).carrier,
..(S₁ ∩ S₂).subset_comm_ring }

instance subring.Inter {ι : Sort*} (S : ι → subring R) /-[h : ∀ y : ι, is_subring (S y)]-/ :
  subring R :=
{carrier := (set.Inter S) }

lemma is_subring_Union_of_directed {ι : Type*} [hι : nonempty ι]
  (s : ι → subring R) [∀ i : ι, (s i) : subring R ]
